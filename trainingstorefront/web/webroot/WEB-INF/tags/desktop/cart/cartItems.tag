		...
		<tbody>
			<c:forEach items="${cartData.entries}" var="entry" varStatus="loop">
				...
				<tr class="cartItem">
					...
					<td headers="header3" class="itemPrice"><format:price
							priceData="${entry.basePrice}" displayFreeForZero="true" /></td>

					<c:choose>
						<c:when test="${not entry.product.multidimensional}">
							<td headers="header4" class="quantity"><c:url
									value="/cart/update" var="cartUpdateFormAction" /> <form:form
									id="updateCartForm${loop.index}"
									...
									<c:if test="${entry.updateable}">
										<ycommerce:testId code="cart_product_updateQuantity">
											<a href="#" id="QuantityProduct_${loop.index}"
									...
									<ycommerce:testId code="cart_product_removeProduct">
										...
										<a href="#" id="RemoveProduct_${loop.index}"
						...
						<c:otherwise>
							<td headers="header4" class="quantity"><c:url
									value="/cart/updateMultiD" var="cartUpdateFormAction" /> <form:form
									id="updateCartForm${loop.index}"
									action="${cartUpdateFormAction}" method="post"
									commandName="updateQuantityForm${entry.entryNumber}">
									<input type="hidden" name="entryNumber"
									...
									<ycommerce:testId code="cart_product_updateQuantity">
										<div id="QuantityProduct${loop.index}"
											class="updateQuantityProduct"></div>
									</ycommerce:testId>
								</form:form> <c:if test="${entry.updateable}">
									<ycommerce:testId code="cart_product_removeProduct">
										<spring:theme code="text.iconCartRemove" var="iconCartRemove" />
										<a href="#" id="RemoveProduct" data-index="${loop.index}"
											class="submitRemoveProductMultiD">${iconCartRemove}</a>
									</ycommerce:testId>
								</c:if></td>
						</c:otherwise>
					</c:choose>
					...
					<c:if test="${ycommerce:checkIfPickupEnabledForStore() eq true}">
						<td headers="header5" class="shipping"><c:url
								value="/store-pickup/cart/update/delivery"
								var="cartEntryShippingModeAction" /> <form:form
								id="cartEntryShippingModeForm_${entry.product.code}${entry.entryNumber}"
								class="cartForm cartEntryShippingModeForm clear_fix"
								action="${cartEntryShippingModeAction}" method="post">
								<input type="hidden" name="entryNumber"
									value="${entry.entryNumber}" />
									...
									<c:when test="${entry.product.purchasable}">
										<c:if
											...
											<label for="shipMode${entry.entryNumber}" class="nostyle">
												<input type="radio" name="shipMode" value="ship"
												id="shipMode${entry.entryNumber}"
										...
										<c:choose>
											<c:when test="${entry.product.availableForPickup}">
												<label for="shipModePickUp${entry.entryNumber}"
													class="nostyle"> <input type="radio"
													name="shipMode" value="pickUp"
													id="shipModePickUp${entry.entryNumber}"
												...
												<c:set var="hideChangeStoreLink"
													value="${not canBePickedUp ? 'style=display:none' : ''}" />
												<div ${hideChangeStoreLink} class="changeStoreLink">
													<storepickup:clickPickupInStore product="${entry.product}"
														cartPage="true" entryNumber="${entry.entryNumber}"
														deliveryPointOfService="${entry.deliveryPointOfService.name}"
					...
					<td headers="header7" class="multidimensional"><c:if
							test="${entry.product.multidimensional}">
							<ycommerce:testId code="cart_product_updateQuantity">
								<a href="#" id="QuantityProductToggle"
									data-index="${loop.index}"
									class="updateQuantityProduct updateQuantityProduct-toggle">+</a>
							</ycommerce:testId>
						</c:if></td>
				</tr>

				<tr>
					<td colspan="6"><div id="ajaxGrid${loop.index}"
							style="display: none"></div></td>
				</tr>

				<c:if test="${entry.product.multidimensional}">
					<tr>
						<th colspan="6"><c:forEach items="${entry.entries}"
							...
							<div style="display: none" id="grid${loop.index}"
