	...
	<c:forEach items="${steps}" var="checkoutStep" varStatus="status">
		...
		<c:choose>
			<c:when test="${progressBarId eq checkoutStep.progressBarId}">
				...
				<li class="step active"><a href="${stepUrl}"> <spring:theme
							code="checkout.multi.${checkoutStep.progressBarId}" />
				</a></li>
			</c:when>
			<c:when test="${checkoutStep.stepNumber > currentStepActive }">
				<li class="step disabled"><a href="${stepUrl}"> <spring:theme
							code="checkout.multi.${checkoutStep.progressBarId}" />
				</a></li>
			</c:when>
			<c:otherwise>
				<li class="step visited"><a href="${stepUrl}"> <spring:theme
							code="checkout.multi.${checkoutStep.progressBarId}" />
				</a></li>
			</c:otherwise>
		</c:choose>
