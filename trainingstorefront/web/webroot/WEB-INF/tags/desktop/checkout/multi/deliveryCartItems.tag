...
<table class="deliveryCartItems">
	<thead>
		<tr>
			<td colspan="5"><spring:theme
					code="checkout.pickup.items.to.be.delivered" /></td>
		</tr>
	</thead>
	...
	<tbody>
		<c:forEach items="${cartData.entries}" var="entry" varStatus="loop">
			<c:if test="${entry.deliveryPointOfService == null}">
				...
				<tr>
					<td rowspan="2" class="thumb"><a href="${productUrl}"> <product:productPrimaryImage
				...
				<tr>
					<td class="priceRow"><format:price
							priceData="${entry.basePrice}" displayFreeForZero="true" /></td>
					<td class="priceRow"><spring:theme code="basket.page.qty" />:
						${entry.quantity}</td>
					<td class="priceRow"><format:price
							priceData="${entry.totalPrice}" displayFreeForZero="true" /></td>
					<td class="priceRow multidimensional"><c:if
							test="${entry.product.multidimensional}">
							<a href="#" id="QuantityProductToggle" data-index="${loop.index}"
								class="showQuantityProductOverlay updateQuantityProduct-toggle">+</a>
						</c:if></td>
				</tr>
				<tr>
					<td colspan="5"><div id="ajaxGrid${loop.index}"
