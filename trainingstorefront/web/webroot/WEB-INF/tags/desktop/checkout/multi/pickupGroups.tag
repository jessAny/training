...
<div id="checkoutContentPanel" class="clearfix">
	<div class="headline">
		<spring:theme code="checkout.pickup.pickup.in.store.title" />
	</div>
	<div class="description">
		<spring:theme code="checkout.pickup.confirm.and.continue" />
	</div>
	...
	<c:forEach items="${cartData.pickupOrderGroups}" var="groupData"
		...
		<div class="column contentPanelLeft">
			...
			<store:openingSchedule
				openingSchedule="${groupData.deliveryPointOfService.openingHours}" />
		</div>
		<div class="column contentPanelRight last">
			<multi-checkout:pickupCartItems cartData="${cartData}"
				groupData="${groupData}" index="${status.index}" showHead="false" />
		</div>
	</c:forEach>
</div>
