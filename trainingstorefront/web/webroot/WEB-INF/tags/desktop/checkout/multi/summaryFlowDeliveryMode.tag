...
<div class="summaryDeliveryMode clearfix">
	<ycommerce:testId code="checkout_deliveryModeData_text">
		<c:if test="${cartData.deliveryItemsQuantity > 0}">
			<div class="column append-1">
				<strong><spring:theme
						code="checkout.summary.deliveryMode.header" htmlEscape="false" /></strong>
				<ul>
	...
	<c:if test="${cartData.pickupItemsQuantity > 0}">
		<div class="column">
			<strong>&nbsp;</strong>
			<ul>
				<li><spring:theme code="checkout.pickup.items.to.pickup"
						arguments="${cartData.pickupItemsQuantity}" /></li>
				<li><spring:theme code="checkout.pickup.store.destinations"
						arguments="${fn:length(cartData.pickupOrderGroups)}" /></li>
			</ul>
		</div>
	</c:if>
	...
	<c:if test="${cartData.deliveryItemsQuantity > 0}">
		<ycommerce:testId code="checkout_changeDeliveryMode_element">
			<a href="<c:url value="/checkout/multi/delivery-method/choose"/>"
