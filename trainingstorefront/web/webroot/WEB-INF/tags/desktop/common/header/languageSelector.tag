...
<c:if test="${fn:length(languages) > 1}">
	...
	<form:form action="${setLanguageActionUrl}" method="post"
		id="lang_form_${component.uid}" class="lang-form">
		<spring:theme code="text.language" var="languageText" />
		<div class="control-group">
			<label class="control-label skip"
				for="lang_selector_${component.uid}">${languageText}</label>

			<div class="controls">
				<ycommerce:testId code="header_language_select">
					<select name="code" id="lang_selector_${component.uid}"
						class="lang-selector">
