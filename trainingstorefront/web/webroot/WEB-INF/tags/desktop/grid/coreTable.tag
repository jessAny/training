...
<div class="single_grid_three_dimensions">
	<table class="grid_three_dimensions clear">
		...
		<tr>
			...
			<c:forEach items="${variants}" var="variant">
				<td><format:price
						priceData="${variant.variantOption.priceData}" /> <input
					type=hidden id="productPrice[${loopIndexPrice}]"
					value="${variant.variantOption.priceData.value}" /></td>
		...
		<tr>
			...
			<c:forEach items="${variants}" var="variant">
				...
				<td><input type="hidden" class="${skusId} sku"
					name="cartEntries[${loopIndex}].sku"
					id="cartEntries[${loopIndex}].sku"
					value="${variant.variantOption.code}" /> <input
					type="${inputType}" class="sku-quantity"
					name="cartEntries[${loopIndex}].quantity"
					id="cartEntries[${loopIndex}].quantity" value="0" ${inputDisabled} />
				</td>
