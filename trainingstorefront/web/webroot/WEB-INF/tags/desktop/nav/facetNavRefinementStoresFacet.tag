...
<c:if test="${not empty facetData.values}">
	<div class="facet">
		...
		<div
			class="facetValues <c:if test="${empty userLocation}"> search_location </c:if> <c:if test="${not empty userLocation}"> search_no_location	hidden</c:if> clearfix">
			<form name="userLocationForm" action="${searchUserLocationUrl}"
				method="GET" id="user_location_form" class="storesFacetSearchForm">
				<div class="control-group left">
					<label class="control-label skip" for="user_location_query"><spring:theme
							code="storeFinder.search" /></label>

					<div class="controls">
						<input type="text" name="q" id="user_location_query"
							class="storesFacetSearchInput">
					</div>
				</div>
				<button type="submit" id="user_location_query_button"
					class="storesFacetSearchSubmit right"></button>
			</form>

			<div class="line-text">
				<span><spring:theme code="storeFinder.line.text" /></span>
			</div>
			<form:form name="autoLocationForm" action="${autoUserLocationUrl}"
				method="POST" id="auto_location_form">
				<input type="hidden" id="latitude" name="latitude" />
				<input type="hidden" id="longitude" name="longitude" />
				<button type="submit" id="findStoresNearMeAjax"
					class="positive input-block-level findStoresNearMe">
		...
		<c:if test="${not empty userLocation}">
			<div class="facetValues search_location change_location">
				<a href="#" class="change_location-link neutral"
					onclick="$(this).closest('div.search_location').hide(); $(this).parent().siblings().closest('div.search_no_location').show();$(this).parent().siblings().find('div.allFacetValues').hide();$(this).parent().siblings().find('a.refinementToggle').focus(); return false;">
				...
				<c:if test="${not empty userLocation.searchTerm}">
					<span class="find-stores-near-me user-location-search-term">
						<spring:theme code="search.nav.resultsForStore"
							arguments="${userLocation.searchTerm}" />
					</span>
				</c:if>
			</div>
						...
						<div class="allFacetValues">
							<ul class="facet_block ${facetData.multiSelect ? '' : 'indent'}">
								<c:forEach items="${facetData.values}" var="facetValue"
									varStatus="status">
									<li
										class="${(status.index < 5 or facetValue.selected) ? '' : 'hidden'}">
										<c:if test="${facetData.multiSelect}">
											<form action="#" method="get">
												<input type="hidden" name="q"
													...
													${facetValue.name} <span class="facetValueCount"> <spring:theme
							...
							<c:if test="${fn:length(facetData.values) > 5}">
								<span class="more"> <a href="#"
									class="moreStoresFacetValues"><spring:theme
