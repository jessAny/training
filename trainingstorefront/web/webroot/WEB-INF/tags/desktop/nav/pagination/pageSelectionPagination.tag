...
<ul class="pagination">
	...
	<c:if test="${hasPreviousPage}">
		<li class="first"><spring:url value="${searchUrl}"
				var="firstPageUrl" htmlEscape="true">
				<spring:param name="page" value="0" />
			</spring:url> <ycommerce:testId code="firstPage_link">
				<a rel="prev" href="${firstPageUrl}"><spring:theme
						code="${themeMsgKey}.firstPage" /></a>
			</ycommerce:testId></li>
	</c:if>


	<c:if test="${hasPreviousPage}">
		<li class="prev"><spring:url value="${searchUrl}"
				...
				<a href="${previousPageUrl}" rel="prev"><spring:theme
						code="${themeMsgKey}.linkPreviousPage" /></a>
			</ycommerce:testId></li>
	</c:if>
	...
	<c:if test="${hasNextPage}">
		<li class="last"><spring:url value="${searchUrl}"
				var="lastPageUrl" htmlEscape="true">
				<spring:param name="page"
					value="${searchPageData.pagination.numberOfPages - 1}" />
			</spring:url> <ycommerce:testId code="lastPage_link">
				<a href="${lastPageUrl}" rel="next"><spring:theme
						code="${themeMsgKey}.lastPage" /></a>
			</ycommerce:testId></li>
	</c:if>
