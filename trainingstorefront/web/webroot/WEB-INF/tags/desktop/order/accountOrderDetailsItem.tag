...
<%@ attribute name="consignment" required="true"
	type="de.hybris.platform.commercefacades.order.data.ConsignmentData"%>
<%@ attribute name="inProgress" required="false"
...
<c:choose>
	<c:when test="${not inProgress}">
		<c:if test="${consignment.status.code eq 'READY_FOR_PICKUP'}">
			<h2>
				<spring:theme code="text.account.order.warning.storePickUpItems"
					text="Reminder - Please pick up your items(s) soon." />
			</h2>
		</c:if>
	</c:when>
</c:choose>
...
<div class="orderList">
	...
	<div class="description">
		<c:choose>
			<c:when test="${not inProgress}">
				<strong><spring:theme
						code="text.account.order.consignment.status.${consignment.statusDisplay}" />:</strong>
				<fmt:formatDate value="${consignment.statusDate}" pattern="MM/dd/yy" />
				<c:choose>
					<c:when test="${consignment.status.code eq 'SHIPPED'}">
						<strong> <spring:theme code="text.account.order.tracking"
								text="Tracking #:" />
						</strong>
						<c:choose>
							...
							<c:otherwise>
								<spring:theme
									code="text.account.order.consignment.trackingID.notavailable"
									text="Not available." />
							</c:otherwise>
						</c:choose>
		...
		<c:choose>
			<c:when test="${consignment.status.code eq 'READY_FOR_PICKUP'}">
				<c:set var="pos"
					value="${consignment.entries[0].orderEntry.deliveryPointOfService}" />
				<c:url value="/store/${pos.name}" var="posUrl" />
				<strong> <spring:theme
						code="text.account.order.pickup.location" text="Pick Up Location:" />
				</strong>
	...
	<table class="orderListTable">
		<thead>
			<tr>
				<th id="header1"><span class="hidden"><spring:theme
							code="text.product" text="Product" /></span></th>
				<th id="header2"><span class="hidden"><spring:theme
							code="text.productDetails" text="Product Details" /></span></th>
		...
		<c:forEach items="${consignment.entries}" var="entry">
			...
			<tr class="item">
				<td headers="header1" class="product_image thumb"><a
					href="${productUrl}"> <product:productPrimaryImage
							product="${entry.orderEntry.product}" format="thumbnail" />
				</a></td>
				<td headers="header2" class="product_details"><ycommerce:testId
						code="orderDetails_productName_link">
						<a
							href="${entry.orderEntry.product.purchasable ? productUrl : ''}">${entry.orderEntry.product.name}</a>
					</ycommerce:testId> <c:forEach items="${entry.orderEntry.product.baseOptions}"
						var="option">
						<c:if
							test="${not empty option.selected and option.selected.url eq entry.orderEntry.product.url}">
						...
						<ul>
							<c:forEach items="${order.appliedProductPromotions}"
								...
								<c:forEach items="${promotion.consumedEntries}"
									var="consumedEntry">
									<c:if
										test="${not displayed && consumedEntry.orderEntryNumber == entry.orderEntry.entryNumber}">
