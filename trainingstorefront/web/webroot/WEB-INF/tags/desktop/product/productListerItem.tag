...
<div
	...
	<ycommerce:testId code="test_searchPage_wholeProduct">
		<a href="${productUrl}" title="${product.name}"
			...
			</c:if> <product:productListerClassifications product="${product}" /> <c:set
				var="product" value="${product}" scope="request" /> <c:set
				var="addToCartText" value="${addToCartText}" scope="request" /> <c:set
				var="addToCartUrl" value="${addToCartUrl}" scope="request" /> <c:if
				test="${not product.multidimensional}">
			...
			<c:when test="${product.multidimensional}">
				<div class="cart clearfix">
					...
					<a href="${backToProductUrl}" class="button right"><spring:theme
							code="product.view.details" /></a> <a href="${productOrderFormUrl}"
						class="button right"><spring:theme code="order.form" /></a>
				</div>
			</c:when>
			<c:otherwise>
