...
<div id="write_reviews" class="reviews" style="display: none">
		...
		<form:form method="post" action="${productReviewActionUrl}"
			...
			<div class="write_review_container">
				...
				<spring:bind path="rating">
					<div
						class="control-group<c:if test="${not empty status.errorMessages}"> error</c:if>">
						...
						<div id="stars-wrapper" class="controls clearfix">
							<c:forEach begin="1" end="5" varStatus="status">
								...
								<form:radiobutton path="rating" value="${status.index}" />${status.index}/${status.end}</label>
								<br />
							</c:forEach>
						</div>
				...
				<formElement:formInputBox idKey="alias" labelKey="review.alias"
					path="alias" inputCSS="" mandatory="false" />
			</div>
	...
	<div class="actionBar bottom clearfix">
		<a href="#" id="read_reviews_action"><spring:theme
				code="review.back" /></a>
	</div>
</div>
