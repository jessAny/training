...
<c:if test="${searchPageData ne null and !empty searchPageData.results}">
	<div class="storeResultList">
		<table class="storeResultTable">
			<thead>
				<tr>
					...
					<th id="header2"><spring:theme
							code="storeFinder.table.distance" /></th>
			...
			<tbody>
				<c:forEach items="${searchPageData.results}" var="pos"
					...
					<tr class="storeItem">
						<td headers="header1"><a href="${fn:escapeXml(posUrl)}"
							class="left"> <img src="" alt="" class="storeMarker" /> <ycommerce:testId
									code="storeFinder_result_image">
									<store:storeImage store="${pos}" format="cartIcon" />
								</ycommerce:testId>
						</a> <c:url value="${pos.url}" var="storeUrl" scope="request" /> <action:actions
								element="div" styleClass="itemName"
						...
						<td headers="header2">${pos.formattedDistance}</td>
		...
		<nav:pagination top="false" supportShowAll="false"
