...
<c:set var="hasNextPage"
	value="${(searchPageData.pagination.currentPage + 1) lt searchPageData.pagination.numberOfPages}" />
<c:if test="${hasNextPage}">
	<form action="${pickupInStoreUrl}" id="last_page_storepickup_form"
		method="get">
		<input type="hidden" name="page" id="last_page_value"
			value="${searchPageData.pagination.numberOfPages-1}" />
		<button class="link" type="submit">
			<spring:theme code="pickup.pagination.last" />
		</button>
	</form>
	<form action="${pickupInStoreUrl}" id="next_results_storepickup_form"
		method="get">
		<input type="hidden" name="page" id="next_page_value"
...
<c:forEach begin="${beginPage}" end="${endPage}" var="page">
	...
	<c:choose>
		<c:when
			test="${searchPageData.pagination.currentPage+1 eq pageNumber}">
			<div>
				<button class="link" type="button" disabled="disabled">${pageNumber}</button>
			</div>
		</c:when>
		<c:otherwise>
			<form action="${pickupInStoreUrl}"
				id="page_${pageNumber}_storepickup_form" method="get">
				<input type="hidden" name="page" id="page_${pageNumber}"
					value="${pageNumber-1}" />
...
<c:if test="${searchPageData.pagination.currentPage gt 0}">
	<form action="${pickupInStoreUrl}" id="back_results_storepickup_form"
		method="get">
		<input type="hidden" name="page" id="back_page_value"
			value="${searchPageData.pagination.currentPage-1}" />
		<button class="link" type="submit">
			<spring:theme code="pickup.pagination.previous" />
		</button>
	</form>
	<form action="${pickupInStoreUrl}" id="first_page_storepickup_form"
		method="get">
		<input type="hidden" name="page" id="first_page_value" value="0" />
		<button class="link" type="submit">
			<spring:theme code="pickup.pagination.first" />
		</button>
	</form>
</c:if>