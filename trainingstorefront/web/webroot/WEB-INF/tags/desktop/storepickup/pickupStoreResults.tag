...
<ul class="searchPOSResultsList clear_fix">
	<c:forEach items="${searchPageData.results}" var="pickupStore">
		<li class="column searchPOSResult">
			...
			<div class="resultDistance">${pickupStore.formattedDistance}</div> <c:choose>
				<c:when
				...
				<c:choose>
					<c:when test="${cartPage}">
						<form:form id="selectStoreForm" class="select_store_form"
							...
							<input type="hidden" name="hiddenPickupQty" value="1"
								class="hiddenPickupQty" />
							<button type="submit"
								class="positive  pickup_here_instore_button">
								<spring:theme code="pickup.here.button" />
							</button>
						</form:form>
					</c:when>
					<c:otherwise>
						<form:form class="add_to_cart_storepickup_form"
							...
							<input type="hidden" name="hiddenPickupQty" value="1"
								class="hiddenPickupQty" />
							<button type="submit"
								class="positive pickup_add_to_bag_instore_button">
