...
<c:forEach items="${cartData.pickupOrderGroups}" var="groupData"
	...
	<ul class="mFormList itemsList productItemListDetailsHolder listview">
		<li class="checkoutOverview-PickUp-Items-Location"><c:choose>
				<c:when test="${summary}">
					<h4>
				...
				<c:otherwise>
					<h3>
						<spring:theme code="checkout.pickup.items.to.pickup"
							arguments="${groupData.quantity}" />
					</h3>
				</c:otherwise>
			</c:choose> <span class="pickup_store_results-entry pickup_store_results-name">${fn:escapeXml(groupData.deliveryPointOfService.name)}</span>
			...
			<c:if test="${not summary}">
				<span class="pickup_store_results-entry pickup_store_results-total">
					<span class="pickup_store_results-total-label"><spring:theme
							code="checkout.pickup.estimated.total" /></span> <format:price
						priceData="${groupData.totalPriceWithTax}"
						displayFreeForZero="true" />
				</span>
			</c:if></li>

		<checkout:cartItemsForPickUp cartData="${cartData}"
			groupData="${groupData}" />

		<li class="checkoutOverview-PickUp-Items-Toggle"><a href="#"
			data-role="button" class="toggleItemsButton"
			data-toggle-text="<spring:theme code="mobile.checkout.items.hide" />">
