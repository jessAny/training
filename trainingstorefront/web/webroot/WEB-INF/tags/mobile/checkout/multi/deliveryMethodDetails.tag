...
<%@ attribute name="isSelected" required="false"
	type="java.lang.Boolean"%>
<%@ attribute name="isSelectable" required="false"
	type="java.lang.Boolean"%>
	...
	<c:choose>
		<c:when test="${deliveryMethodOk}">
			<c:choose>
				...
				<c:otherwise>
					<div>
						<span>${deliveryMethod.name}</span><br /> <span>${deliveryMethod.description}</span><br />
						<span>${deliveryMethod.deliveryCost.formattedValue}</span><br />
					</div>
				</c:otherwise>
			</c:choose>
		</c:when>
		<c:otherwise>
			<div>
				<spring:theme
					code="checkout.multi.deliveryMethod.noExistingDeliveryMethod" />
			</div>
		</c:otherwise>
	</c:choose>
</div>
