...
<c:choose>
	<c:when test="${not empty deliveryMethods}">
		<form:form id="selectDeliveryMethodUrl"
			action="${selectDeliveryMethodUrl}" method="get">
			<fieldset data-role="controlgroup" data-theme="d">
				<c:forEach items="${deliveryMethods}" var="deliveryMethod">
					<multi-checkout:deliveryMethodDetails id="${deliveryMethod.code}"
			...
			<c:if test="${not empty selectedDeliveryMethodId}">
				<button type="submit" data-icon="arrow-r" data-iconpos="right"
					data-theme="b" class="onefullWidth show_processing_message">
	...
	<c:otherwise>
		<ul>
			<li><spring:theme
					code="checkout.multi.deliveryMethod.noExistingDeliveryMethod" /></li>
		</ul>
	</c:otherwise>
</c:choose>
