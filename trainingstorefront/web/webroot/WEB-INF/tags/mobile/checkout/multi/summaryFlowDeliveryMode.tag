...
<c:set var="pickupDestinations"
	value="${fn:length(cartData.pickupOrderGroups)}" />

<div data-theme="b" data-role="content">
	<ycommerce:testId code="checkout_deliveryModeData_text">
		<div>
			<h4 class="subItemHeader">
				<spring:theme code="checkout.summary.deliveryMode.header"
					htmlEscape="false" />
				<span></span>
			</h4>
		</div>
				...
				<ul class="mFormList">
					<c:if test="${deliveryItems > 0}">
						...
						<li class="deliverymode-description"
							title="${deliveryMode.description} - ${deliveryMode.deliveryCost.formattedValue}">
							${deliveryMode.description}&nbsp;-&nbsp;${deliveryMode.deliveryCost.formattedValue}
						</li>
					</c:if>
					<c:if test="${deliveryItems > 0 && pickupItems > 0}">
						</br>
					</c:if>
					<c:if test="${pickupItems > 0}">
						<li><spring:theme
								code="checkout.summary.deliveryMode.items.for.pickup"
								arguments="${pickupItems}" /></li>
						<li><spring:theme
								code="checkout.summary.deliveryMode.number.of.pickup.destinations"
								arguments="${pickupDestinations}" /></li>
					</c:if>
				</ul>
			</div>
			<div class="ui-block-b" style="width: 15%">
				<c:if test="${deliveryItems > 0}">
					<ycommerce:testId code="checkout_changeDeliveryMode_element">
						...
						<a href="${chooseDeliveryMethodUrl}" data-theme="c"> <spring:theme
