...
<c:if test="${not empty pickupConsolidationOptions}">
	...
	<div class="span-20">
		<div class="selectDeliverylocation">
			<form:form id="selectDeliverylocationForm"
				action="${consolidatePickupUrl}" method="POST">
				<h3>
					<spring:theme
						code="checkout.pickup.items.available.at.one.location" />
				</h3>
				<div class="selectDeliverylocationDescText">
				...
				<fieldset data-role="controlgroup" data-theme="d">
					<c:forEach items="${pickupConsolidationOptions}" var="option"
						...
						<div data-theme="d" class="selectDeliverylocation-list">
							<input class="selectDeliverylocationItemOption" type="radio"
								name="posName" value="${option.name}" id="${option.name}"
								data-theme="d"
								<c:if test='${status.first}'>checked="checked"</c:if> /> <label
								class="selectDeliverylocationItemLabel" for="${option.name}">
								<ul class="mFormList pickupAdressList">
									<c:if test="${not empty userLocation}">
										<li class="pickupOptionDistance">${option.formattedDistance}</li>
									</c:if>
				...
				<div class="selectDeliverylocationItem">
					<span class="selectDeliverylocationDescText"><spring:theme
							code="checkout.pickup.items.simplify.pickup.location" /></span>
