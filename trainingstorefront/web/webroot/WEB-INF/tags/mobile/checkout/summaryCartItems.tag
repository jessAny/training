...
<div data-theme="b" data-role="content">
	<div data-theme="b">
		<div>
			<span class="cart_id"> <spring:theme code="basket.page.cartId" />
			...
			<c:if test="${cartData.deliveryItemsQuantity gt 0}">
				...
				<c:if
					test="${ycommerce:checkIfPickupEnabledForStore() && cartData.pickupItemsQuantity gt 0 && !summary}">
					<span class="checkout-overview-pickup-info"> <spring:theme
							code="basket.page.title.yourDeliveryItems.pickup" />
					</span>
				</c:if>
			</c:if>
		</div>
