...
<c:if test="${not empty accConfMsgs}">
	<div id="accConfMsgs" style="display: none;"
		data-headertext='<spring:theme code="text.headertext.conf"/>'>
		<ul class='success mFormList'>
			<c:forEach items="${accConfMsgs}" var="msg">
				<li><spring:theme code="${msg.code}"
						arguments="${msg.attributes}" /></li>
			</c:forEach>
		</ul>
	</div>
</c:if>
...
<c:if test="${not empty accInfoMsgs}">
	<div id="accInfoMsgs" style="display: none;"
		data-headertext='<spring:theme code="text.headertext.info"/>'>
...
<c:if test="${not empty accErrorMsgs}">
	<div id="accErrorMsgs" style="display: none;"
