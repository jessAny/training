...
<c:if test="${not empty previousPage}">
	<a href="#${previousPage}" id="${leftButtonId}" data-role="button"
		data-icon="arrow-l" data-transition="${pageTransition}"> <spring:theme
			code="search.nav.refine.button" />
	</a>
</c:if>
<h3>
	<spring:theme code="${heading}hi" />
</h3>
<a href="#" data-role="button" id="${rightButtonId}" data-icon="check"><spring:theme
