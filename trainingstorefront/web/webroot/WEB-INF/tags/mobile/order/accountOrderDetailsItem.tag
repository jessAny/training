	...
	<c:when test="${not inProgress}">
		<div class="productItemHeader">
			<c:if test="${consignment.status.code eq 'READY_FOR_PICKUP'}">
				<span class="pick-up-reminder"><spring:theme
						code="text.account.order.warning.storePickUpItems"
						text="Reminder - Please pick up your items(s) soon." /></span>
			</c:if>
			<h3>
				<spring:theme
					code="text.account.order.consignment.status.${consignment.statusDisplay}" />
				:</strong>
			...
			<c:choose>
				<c:when test="${consignment.status.code eq 'SHIPPED'}">
					<div class="ui-grid-a productItemFullPusher">
						<h5>
							<spring:theme code="text.account.order.tracking"
								text="Tracking #:" />
							<c:choose>
...
<c:choose>
	<c:when test="${consignment.status.code eq 'READY_FOR_PICKUP'}">
		...
		<div class="ui-grid-a productItemFullPusher" data-theme="b">
			<h4>
				<spring:theme code="text.account.order.pickup.location"
					text="Pick Up Location:" />
			</h4>
			<ul class="mFormList itemsList productItemListDetailsHolder listview">
				<li class="checkoutOverview-PickUp-Items-Location"><span
					class="pickup_store_results-entry pickup_store_results-name">${pos.name}</span>
