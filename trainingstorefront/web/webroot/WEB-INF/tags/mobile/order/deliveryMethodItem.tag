...
<div data-theme="b" data-role="content">
	...
	<c:if test="${fn:length(order.pickupOrderGroups) gt 0}">
		<div data-theme="d">
			<ul class="mFormList">
				<li><spring:theme code="checkout.orderConfirmation.pickupItems"
						text="{0} Items for Pick Up in Store"
						arguments="${order.pickupItemsQuantity}" /></li>
				<li><spring:theme
						code="checkout.orderConfirmation.pickupPoints"
						text="{0} Store Pick Up Destination(s)"
						arguments="${fn:length(order.pickupOrderGroups)}" /></li>
			</ul>
		</div>
	</c:if>
</div>
