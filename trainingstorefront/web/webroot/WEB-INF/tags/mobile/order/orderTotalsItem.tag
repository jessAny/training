...
<%@ attribute name="hideHeading" required="false"
...
<div class="orderTotalFullPusher" id="cartTotals" data-theme="b"
	data-role="content">
	...
	<c:if test="${not order.net}">
		<p>
			<spring:theme code="text.account.order.includesTax"
				text="Your order includes {0} tax"
				arguments="${order.totalTax.formattedValue}" argumentSeparator="###" />
		</p>
	</c:if>
