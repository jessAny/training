...
<%@ attribute name="allowAddToCart" required="true"
...
<div class="prod_add_to_cart" data-theme="b">
	...
	<div class="prod_add_to_cart_submit">
					...
					<c:choose>
						<c:when
							...
							<label for="qty" class="skip"> <spring:theme
									code="basket.page.quantity" />
							</label>
							<select id="qty" class='qty qtySelector' name="qty"
								data-theme="d">
								<option value="1">
					...
					<c:when test="${not empty variantSizes}">
						<c:choose>
							<c:when
								...
								<c:if
									test="${product.stock.stockLevelStatus.code == 'lowStock' and showAddToCart}">
									<div id="outOfStock">
							...
							<c:when
								test="${(sizeSelected == false) and not empty variantSizes}">
								<div class="productprice">
									<format:price priceData="${product.price}" />
								</div>
								<div id="stockLevel">
							...
							<c:otherwise>
								...
								<c:if
									test="${product.stock.stockLevelStatus.code == 'lowStock'}">
									<div id="outOfStock">
										<spring:theme code="product.variants.only.left"
											arguments="${product.stock.stockLevel}" />
									</div>
		...
		<div id="actions-container-for-${component.uid}"
			class="productAddToCartPanelContainer clearfix">
			<action:actions />
		</div>
