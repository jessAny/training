...
<c:set value="${ycommerce:productImage(product, 'cartIcon')}"
	var="thumbnailImage" />
<c:if test="${not empty galleryImages}">
	<div class="imageGallery swipeGallery" data-enabled='true'
		class="hasThumbnail" data-commonresourcepath="${commonResourcePath}"
		data-thumbnail="${thumbnailImage.url}">
		<c:forEach items="${galleryImages}" var="container"
			...
			<div class='imageGallerySlide swipeGallerySlide'
				...
				data-title="${product.name}" data-alt="${product.name}"
				data-zoom='in'>
				<div class="imageContainer">
					<img src="${container.product.url}" title="${product.name}"
						alt="${product.name}" />
				</div>
				<div class='zoomButtonOverlay'>
					<div class='zoomButton'>
						<div class='zoomInButton'
							alt='<spring:theme code="general.zoom"/>' tabindex='0'></div>
					</div>
				</div>
			</div>
		</c:forEach>
...
<c:if test="${empty galleryImages}">
	<div>
		<theme:image code="img.missingProductImage.product"
			alt="Missing product image" title="Missing product image" />
	</div>
</c:if>
