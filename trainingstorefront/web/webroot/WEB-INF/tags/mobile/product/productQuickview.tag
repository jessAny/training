...
<form:form id="addToCartForm" class="add_to_cart_form"
	action="${addToCartUrl}" method="post">
	<div class='ui-grid-a accmob-quickviewItemInfo'>
		<div class='ui-block-a'>
			<a href='<c:url value="${product.url}"/>'
				data-url="${productQuickViewUrl}" data-rel="dialog"
				data-transition="pop"> <mproduct:productPrimaryImage
					product="${productReference.target}" format="thumbnail"
					zoomable='false' />
			</a>
		</div>
		<div class='ui-block-b'>
			<a href='<c:url value="${product.url}"/>'> <spring:theme
					code="product.product.details.more" /></a>
			<%-- promotions --%>
			...
			<span class="accmob-quickviewItemPrice"><format:price
					priceData="${product.price}" /></span>
		</div>
	</div>
	<c:set var="purchasable"
		value="${product.purchasable and product.stock.stockLevelStatus.code ne 'outOfStock'}" />
	<c:set var="disabled"
		value="${purchasable ? '' : 'disabled=\"disabled\"'}" />
		...
		<div class='ui-block-a'>
			<div class="prod_add_to_cart_quantity">
				<label for="qty" class="skip"><spring:theme
					...
					name="qty" data-theme="d" ${disabled}>
				...
				</select> <input type="hidden" name="productCodePost" value="${product.code}" />
			</div>
		</div>
		<div class="ui-block-b">
			<div class="accmob-quickviewItemAvailability">
				<c:choose>
					<c:when test="${purchasable}">
						${product.stock.stockLevel} &nbsp; <spring:theme
	...
	<div id='addToBasket'>
		...
		<button type="${buttonType}" data-rel="dialog" data-transition="pop"
			data-theme="b" class="positive large" ${disabled}>
			<spring:theme
				code="text.addToCart${purchasable ? '' : '.outOfStock'}" />
		</button>
		<c:if test="${product.availableForPickup}">
			<a href="#" class="pickUpInStoreButton"
				data-productcode="${product.code}" data-rel="dialog"
				data-transition="pop" data-role="button" data-theme="c"> <spring:theme
