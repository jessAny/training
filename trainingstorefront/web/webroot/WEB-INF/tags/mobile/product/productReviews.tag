...
<div data-role="collapsible" data-theme="e" data-content-theme="c"
	data-collapsed="true" data-icon="arrow-u" class="review_detail"
	id='review_detail'>
	<h3>
		<spring:theme code="text.productreviews" />
	</h3>
	<p>
	<div id="read_reviews">
		<c:if test="${not empty product.reviews}">
			<ul class="productReviews" data-role="listview" data-theme="e">
				<c:forEach items="${product.reviews}" var="review"
					varStatus="status">
					<li data-theme="e" ${status.count >3?"style='display: none'":"" }
						${status.count >3?"class='hiddenReview'":"" }>
						<div class='review'>
							<c:choose>
							...
							<p>
								<span class="stars large right"> <span
									style="width: <fmt:formatNumber maxFractionDigits="0" value="${(review.rating) * 17}" />px;"></span>
								</span>
							</p>
							<div class="review_detail" style="${reviewDetailStyle}">
								<ycommerce:testId code="review_headline">
							...
							<p>
								<ycommerce:testId code="review_was_writtern_by">
									<strong> <spring:theme code="review.submitted.by" />
				...
				<li data-theme="e">
					<div class="ui-grid-a">
						<c:if test="${fn:length(product.reviews) gt 3}">
							<div class="ui-block-a">
								<ycommerce:testId code="show_more_reviews_button">
									<a href="javascript:void('#')" class="showReviews"
										data-theme="d" data-role="button"> <spring:theme
						...
						<div class="ui-block-b"
							${fn:length(product.reviews) le 3 ? "style=width:100%":""}>
							<c:url value="/p" var="encodedUrl" />
							<a href="${encodedUrl}/${product.code}/writeReview"
								data-role="button" data-theme="c"> <c:choose>
		...
		<c:if test="${empty product.reviews}">
			<c:url value="/p" var="encodedUrl" />
			<a href="${encodedUrl}/${product.code}/writeReview"
				data-role="button" data-theme="c"> <spring:theme
					code="review.no.reviews" />
			</a>
		</c:if>
	</div>
	</p>
