	...
	<c:forEach items="${variantStyles}" var="variantStyle">
		<c:forEach items="${variantStyle.variantOptionQualifiers}"
			...
			<c:if test="${variantOptionQualifier.qualifier eq 'style'}">
				...
				<c:if test="${not empty styleValue }">
				...
				<select id="colorSelector" class='variantSelector' data-theme="d">
					<c:forEach items="${variantStyles}" var="variantStyle">
						...
						<c:if test="${styleValue != ''}">
							<option value="<c:url value="${variantStyle.url}"/>"${(variantStyle.url eq currentStyleUrl) ? 'selected="selected"' : ''}">${styleValue}</option>
						</c:if>
					</c:forEach>
				</select>
			</div>
		...
		<div class="ui-grid-a variant">
			...
			<select id="Size" class='variantSelector' data-theme="d">
				<c:if test="${empty variantSizes}">
					<option
						<c:if test="${empty variantParams['size']}">selected="selected"</c:if>><spring:theme
				...
				<c:if test="${not empty variantSizes}">
					...
					<c:forEach items="${variantSizes}" var="variantSize">
						...
						<c:if
							test="${(empty variantSize.stock.stockLevel) and (variantSize.stock.stockLevelStatus eq 'inStock')}">
						...
						<option value="${variantSizeUrl}"
							${(variantSize.url eq product.url) ? 'selected="selected"' : ''}>${optionsString}&nbsp;${stockLevel}</option>
						<c:if test='${(variantSize.url eq product.url)}'>
		...
		<div class="size">
			<select id="variant" class='variantSelector' data-theme="d">
				...
				<c:forEach items="${variantOptions}" var="variantOption">
					...
					<option value="<c:url value="${variantOption.url}"/>"
						${(variantOption.url eq product.url) ? 'selected="selected"' : ''}>${optionsString}
						&nbsp;
