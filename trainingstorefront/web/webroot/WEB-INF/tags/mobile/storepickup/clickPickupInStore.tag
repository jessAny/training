...
<%@ taglib prefix="storepickup"
	tagdir="/WEB-INF/tags/mobile/storepickup"%>
...
<%@ attribute name="deliveryPointOfService" required="false"
	type="java.lang.String"%>
<%@ attribute name="quantity" required="false" type="java.lang.Integer"%>
<%@ attribute name="searchResultsPage" required="false"
	type="java.lang.Boolean"%>
<%@ taglib prefix="form" uri="http://www.springframework.org/tags/form"%>


<c:url var="pickupInStoreUrl"
	value="/store-pickup/${product.code}/pointOfServices" />

<div class="collect_from_store">
	<c:choose>
		<c:when test="${cartPage}">
			<a href="javascript:void(0)" class="click_pickupInStore_Button"
				id="product_${product.code}${entryNumber}"> <c:choose>
...
<div id="popup_store_pickup_form_product_${product.code}${entryNumber}"
	class="pickup_store_search" style="display: none">
	...
	<div class="prod_grid span-4">
		<span class="thumb"> <product:productPrimaryImage
				product="${product}" format="thumbnail" />
		</span> <span class="details"> <strong class="strong prod_grid-name">${product.name}</strong>
		</span>
		<div class="cart">
		...
		<div class="quantity pickup_store_search-quantity">
			<label for="pickupQty"><spring:theme
					code="basket.page.quantity" /></label> <input type="text" size="1"
				id="pickupQty" name="qty" class="qty"
				<c:choose>
					<c:when test="${cartPage}">
						value="${quantity}"
					</c:when>
	...
	<div class="span-17 last">
		<div class="pickup_store_search-form">
			<form:form name="pickupInStoreForm" action="${pickupInStoreUrl}"
				method="post"
				id="pickup_in_store_search_form_product_${product.code}${entryNumber}"
				class="form_field-input">
				<spring:theme code="pickup.search.message" />
				<input type="text" name="locationQuery" id="locationForSearch" />
				<input type="hidden" name="cartPage" id="atCartPage"
					value="${cartPage}" />
				<input type="hidden" name="entryNumber" value="${entryNumber}"
					class="entryNumber" />
				<button type="submit" class="form" id="pickupstore_search_button">
