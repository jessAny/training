...
<ul data-role="listview" data-theme="d"
	...
	<c:choose>
					...
					<div class="ui-grid-60-40 clearfix ${outOfStockStatus}">
						<div class="ui-block-a">
							<c:choose>
								<c:when
									test="${pickupStore.stockData.stockLevelStatus.code eq 'outOfStock'}">
									<span class="pickup_store_results-list_stock_out"> <spring:theme
											code="pickup.out.of.stock" />
									</span>
								</c:when>
								<c:when
									test="${pickupStore.stockData.stockLevelStatus.code ne 'outOfStock' and empty pickupStore.stockData.stockLevel}">
									<span class="pickup_store_results-list_stock_in"> <spring:theme
											code="pickup.force.in.stock" />
									</span>
								</c:when>
								<c:otherwise>
									<span class="pickup_store_results-list_stock_level"> <spring:theme
											code="pickup.in.stock"
											arguments="${pickupStore.stockData.stockLevel}" />
									</span>
								</c:otherwise>
							</c:choose>
						...
						<div class="ui-block-b">
							...
							<c:if
								test="${pickupStore.stockData.stockLevel gt 0 or empty pickupStore.stockData.stockLevel}">
								<c:set var="actionUrl"
									value="${cartPage ? updateSelectStoreUrl : addToCartToPickupInStoreUrl}" />
								<c:set var="formClass"
									value="${cartPage ? 'select_store_form' : 'add_to_cart_storepickup_form'}" />
								<c:set var="buttonMessageCode"
									value="${cartPage ? 'pickup.here.button' : 'pickup.results.button'}" />

								<form:form class="${formClass}" action="${actionUrl}"
									method="post">
									<input type="hidden" name="storeNamePost"
										value="${pickupStore.name}" />
									<input type="hidden" name="hiddenPickupQty"
										class="hiddenPickupQty" value="1" />
									<c:choose>
										...
										<c:otherwise>
											<input type="hidden" name="productCodePost"
												value="${searchPageData.product.code}" />
										</c:otherwise>
									</c:choose>
									<button type="submit" data-role="button" data-theme="b"
										class="positive large pickup_here_instore_button">
		...
		<c:when test="${searchPageData.pagination.totalNumberOfResults eq 0}">
			<li><p>
					<spring:theme code="text.storefinder.mobile.page.noResults" />
				</p></li>
		</c:when>
	</c:choose>
</ul>
