...
<ul data-role="mFormList" data-inset="true" data-theme="e"
	data-content-theme="e">
	<c:if test="${fn:length(languages) > 1}">
		<li><form:form action="${languageUrl}" method="post"
				id="lang-form">
					...
					<select name="code" id="lang-selector" data-theme="f">
						<c:forEach items="${languages}" var="lang">
							<option value="${lang.isocode}"
								${lang.isocode == currentLanguage.isocode ? 'selected="selected"' : ''}
	...
	<c:if test="${fn:length(currencies) > 1}">
		<li><form:form action="${currencyUrl}" method="post"
				id="currency-form">
					...
					<select name="code" id="currency-selector" data-theme="f">
						<c:forEach items="${currencies}" var="curr">
							<option value="${curr.isocode}"
								${curr.isocode == currentCurrency.isocode ? 'selected="selected"' : ''}>${curr.symbol}
								${curr.isocode}</option>
						</c:forEach>
					</select>
				</ycommerce:testId>
