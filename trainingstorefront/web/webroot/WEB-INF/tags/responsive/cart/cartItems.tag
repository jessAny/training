...
<ul class="product-list">
	<c:forEach items="${cartData.entries}" var="entry" varStatus="loop">
		...
		<li class="product-item"><c:if test="${entry.updateable}">
				<ycommerce:testId code="cart_product_removeProduct">
					<c:choose>
						<c:when test="${not entry.product.multidimensional}">
							<button class="btn  remove-item remove-entry-button"
								id="removeEntry_${loop.index}">
								<span class="glyphicon glyphicon-remove"></span>
							</button>
						</c:when>
						<c:otherwise>
							<button class="btn  remove-item submitRemoveProductMultiD"
								data-index="${loop.index}" id="removeEntry_${loop.index}">
								<span class="glyphicon glyphicon-remove"></span>
							</button>
						</c:otherwise>
					</c:choose>
								...
								<div class="radio-column">
									<c:if
										...
										<c:if
											...
											<label for="pick0_${entry.entryNumber}"> <span
												class="glyphicon glyphicon-gift text-gray"></span> <span
												class="name"><spring:theme
														code="basket.page.shipping.ship" /></span>
											</label>
										</c:if>
									</c:if>
									<c:if test="${not empty entry.deliveryPointOfService.name}">
										<label for="pick1_${entry.entryNumber}"> <span
											class="glyphicon glyphicon-home"></span> <span class="name"><spring:theme
								...
								<div class="store-column">
									<c:choose>
										<c:when test="${entry.product.availableForPickup}">
											<c:choose>
												<c:when
													test="${not empty entry.deliveryPointOfService.name}">
					...
					<c:if test="${entry.product.multidimensional}">
						<ycommerce:testId code="cart_product_updateQuantity">
							...
							<c:set var="showEditableGridClass"
								value="with-editable-grid showEditableGrid" />
						</ycommerce:testId>
					</c:if>
								...
								<c:when test="${not entry.product.multidimensional}">
									...
									<form:form id="updateCartForm${loop.index}"
										...
										<ycommerce:testId code="cart_product_quantity">
											...
											<form:input
												cssClass="form-control update-entry-quantity-input"
												disabled="${not entry.updateable}" type="text" size="1"
												id="quantity_${loop.index}" path="quantity" />
										</ycommerce:testId>
									</form:form>
								</c:when>
								<c:otherwise>
									...
									<form:form id="updateCartForm${loop.index}"
										...
										commandName="updateQuantityForm${loop.index}">
										<input type="hidden" name="entryNumber"
											value="${entry.entryNumber}" />
										<input type="hidden" name="productCode"
											value="${entry.product.code}" />
		...
		<div id="ajaxGrid${loop.index}"
			class="add-to-cart-order-form-wrap cartOrderGridFormWrapper"
			style="display: none"></div>
		<c:if test="${entry.product.multidimensional}">
			<c:forEach items="${entry.entries}" var="currentEntry"
				varStatus="stat">
			...
			<div style="display: none" id="grid${loop.index}"
				data-sub-entries="${subEntries}"></div>
		</c:if>
	</c:forEach>
</ul>
