			...
			<div class="text-${textPos}">
				...
				<c:choose>
					<c:when test="${not showModifications}">
						<div class="container">
							<div
								class="col-xs-6 col-sm-4 col-md-3 col-lg-2 col-xs-offset-3 col-sm-offset-4 col-md-offset-4 col-lg-offset-5">
								<br />
								<button class="btn btn-default btn-block cartRestoration"
									data-cart-url="${cartUrl}">
									<spring:theme text="View Cart"
										code="basket.restoration.view.cart.btn" />
								</button>
							</div>
						</div>
					</c:when>
					<c:otherwise>
						...
						<c:forEach items="${restorationData.modifications}"
							...
							<c:choose>
								...
								<c:when
									test="${not modification.deliveryModeChanged and not empty modification.entry}">
									<spring:theme
										code="basket.restoration.${modification.statusCode}"
										arguments="${modification.entry.product.name},${entryUrl},${modification.entry.quantity},${modification.quantityAdded}" />
								</c:when>
								<c:when
									test="${not modification.deliveryModeChanged and empty modification.entry}">
									<spring:theme
										code="basket.restoration.${modification.statusCode}"
										arguments="${modification.entry.product.name},${entryUrl},${modification.quantity},${modification.quantityAdded}" />
								</c:when>
