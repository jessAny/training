...
<c:if test="${hasShippedItems}">
	<div class="checkout-shipping-items row">
			...
			<ul>
				<c:forEach items="${cartData.entries}" var="entry">
					<c:if test="${entry.deliveryPointOfService == null}">
						...
						<li class="row"><span class="name col-xs-8">${entry.product.name}</span>
							<span class="qty col-xs-4"><spring:theme
									code="basket.page.qty" />:&nbsp;${entry.quantity}</span></li>
					</c:if>
				</c:forEach>
			</ul>
		</div>
		...
		<c:if test="${showDeliveryAddress and not empty deliveryAddress}">
			<div class="col-sm-12 col-lg-6">
				<div class="checkout-shipping-items-header">
					<spring:theme code="checkout.summary.shippingAddress"></spring:theme>
				</div>
				<span> <b>${fn:escapeXml(deliveryAddress.title)}&nbsp;${fn:escapeXml(deliveryAddress.firstName)}&nbsp;${fn:escapeXml(deliveryAddress.lastName)}</b>
					<br /> <c:if test="${ not empty deliveryAddress.line1 }">
                        ${fn:escapeXml(deliveryAddress.line1)},&nbsp;
                    </c:if> <c:if test="${ not empty deliveryAddress.line2 }">
                        ${fn:escapeXml(deliveryAddress.line2)},&nbsp;
                    </c:if> <c:if test="${not empty deliveryAddress.town }">
						...
						test="${ not empty deliveryAddress.phone }">
