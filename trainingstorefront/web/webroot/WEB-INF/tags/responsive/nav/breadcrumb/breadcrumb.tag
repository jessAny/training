...
<ol class="breadcrumb">
	<li><a href="${homeUrl}"><spring:theme code="breadcrumb.home" /></a>
	</li>
	...
	<c:forEach items="${breadcrumbs}" var="breadcrumb" varStatus="status">
		...
		<c:choose>
			...
			<c:when test="${breadcrumb.url eq '#'}">
				<li><a href="#">${fn:escapeXml(breadcrumb.name)}</a></li>
			</c:when>
			<c:otherwise>
				<li><a href="${breadcrumbUrl}">${fn:escapeXml(breadcrumb.name)}</a>
				</li>
			</c:otherwise>
		</c:choose>
	</c:forEach>
