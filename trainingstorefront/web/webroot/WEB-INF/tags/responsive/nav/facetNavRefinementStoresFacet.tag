	...
	<ycommerce:testId code="facetNav_title_${facetData.name}">
		<div class="facet js-facet js-shop-stores-facet">
			...
			<div class="facet-values js-facet-values">
				...
				<div
					class="facet-form js-facet-form <c:if test="${not empty userLocation}">facet-form-hidden</c:if>"
					...
					<form name="userLocationForm" action="${searchUserLocationUrl}"
						...
						<div class="input-group">
							<label for="user_location_query" class="sr-only"><spring:theme
									code="storeFinder.search" /></label> <input type="text"
								class="form-control js-shop-store-search-input" name="q"
								id="user_location_query" placeholder=""> <span
					...
					<button type="button" id="findStoresNearMeAjax"
				...
				<div
					class="facet-container js-facet-container <c:if test="${empty userLocation}">facet-container-hidden</c:if>">
					<c:if test="${not empty userLocation}">
						<div class="facet-search">
							<a href="#" class="js-facet-change-link"> <spring:theme
									code="search.nav.changeLocation" />
							</a>
						...
						<c:if test="${not empty facetData.values}">
							<ul
								class="facet-list js-facet-list ${facetData.multiSelect ? '' : 'indent'}">
										...
										<c:if test="${facetData.multiSelect}">
											<form action="#" method="get">
												<input type="hidden" name="q"
													...
													<input class="facet-checkbox js-facet-checkbox sr-only"
													type="checkbox"
													${facetValue.selected ? 'checked="checked"' : ''}>
													<span class="facet-label"> <span class="facet-mark"></span>
														<span class="facet-text"> ${facetValue.name} <ycommerce:testId
																code="facetNav_count">
											...
											<span class="facet-text"> <a
												href="${facetValueQueryUrl}">${facetValue.name}</a> <ycommerce:testId
													code="facetNav_count">
													<span class="facet-value-count"> <spring:theme
															code="search.nav.facetValueCount"
															arguments="${facetValue.count}" /></span>
							...
							<c:if test="${fn:length(facetData.values) > 5}">
								<span class="more"> <a href="#"
									class="js-more-stores-facet-values"><spring:theme
