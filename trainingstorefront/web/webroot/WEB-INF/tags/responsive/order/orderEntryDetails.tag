...
<c:set var="entryStock"
	value="${orderEntry.product.stock.stockLevelStatus.code}" />
		...
		<div class="col-md-12">
			...
			<div class="details">
				...
				<div>
					...
					: <span class="price"> <ycommerce:testId
							...
							<c:choose>
								...
								<c:otherwise>
									<format:price
										priceData="${orderEntry.product.priceRange.minPrice}"
										displayFreeForZero="true" />
                                        -
                                        <format:price
										priceData="${orderEntry.product.priceRange.maxPrice}"
										displayFreeForZero="true" />
								</c:otherwise>
							</c:choose>
						</ycommerce:testId>
...
<c:if test="${orderEntry.product.multidimensional}">
	<c:forEach items="${orderEntry.entries}" var="currentEntry"
		varStatus="stat">
		<c:set var="subEntries"
			value="${stat.first ? '' : subEntries}${currentEntry.product.code}:${currentEntry.quantity}," />
	</c:forEach>

	<div style="display: none" id="grid${itemIndex}"
		data-sub-entries="${subEntries}"
		data-order-code="${fn:escapeXml(order.code)}"
		data-target-url="${targetUrl}"></div>
</c:if>