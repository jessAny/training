...
<%@ attribute name="deliveryPointOfService" required="true"
...
<div class="row">
	<div class="col-sm-6 col-md-4 order-store-address">
		...
		<order:addressItem address="${deliveryPointOfService.address}"
			storeAddress="true" />
		<br /> ${fn:escapeXml(deliveryPointOfService.address.phone)}
	</div>

	<c:set var="openingSchedule"
		value="${deliveryPointOfService.openingHours}" />
	<c:if test="${not empty openingSchedule}">
		...
		<div class="col-sm-6 col-md-4 order-store-hours">
			<c:if test="${not inProgress and statusDate ne null}">
				<div class="label-order">
					<spring:theme code="text.account.order.consignment.pickUpBy" />
				</div>
				<fmt:formatDate value="${statusDate}" dateStyle="medium"
