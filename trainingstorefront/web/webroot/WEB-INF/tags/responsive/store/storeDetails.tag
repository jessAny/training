...
<c:if test="${not empty store.formattedDistance}">
	<div class="detailSection">
		<div class="detailSectionHeadline">
			<spring:theme code="storeDetails.table.distance" />
		</div>
		<c:choose>
			<c:when test="${not empty locationQuery}">
				<spring:theme code="storeDetails.table.distanceFromSource"
					argumentSeparator="^"
					arguments="${store.formattedDistance}^${fn:escapeXml(locationQuery)}" />
			</c:when>
			<c:otherwise>
				<spring:theme code="storeDetails.table.distanceFromCurrentLocation"
					argumentSeparator="^" arguments="${store.formattedDistance}" />
			</c:otherwise>
		</c:choose>
	</div>
</c:if>
...
<c:if test="${not empty store.address.phone}">
	<div class="detailSection">
		<div class="detailSectionHeadline">
			<spring:theme code="storeDetails.table.telephone" />
		</div>
		${store.address.phone}
	</div>
</c:if>
...
<c:if test="${not empty store.address.email}">
	<div class="detailSection">
		<div class="detailSectionHeadline">
			<spring:theme code="storeDetails.table.email" />
		</div>
