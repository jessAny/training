...
<c:if test="${not empty openingSchedule}">
		...
		varStatus="weekDayNumber">

		<c:choose>
			<c:when test="${weekDay.closed}">
						"${weekDay.weekDay}":"<spring:theme
					code="storeDetails.table.opening.closed" />"<c:if
					test="${!weekDayNumber.last}">,</c:if>
			</c:when>
			<c:otherwise>
						"${weekDay.weekDay}":"${weekDay.openingTime.formattedHour} - ${weekDay.closingTime.formattedHour}"<c:if
					test="${!weekDayNumber.last}">,</c:if>
			</c:otherwise>
		</c:choose>
	</c:forEach>
