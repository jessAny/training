...
<%@ attribute name="stockData" required="true"
	type="de.hybris.platform.commercefacades.product.data.StockData"%>
<c:choose>
	...
	<c:when
		test="${stockData.stockLevelStatus.code ne 'outOfStock' and empty stockData.stockLevel}">
	...
	<c:otherwise>
		<div class='resultStock'>
			<spring:theme code="pickup.in.stock"
				arguments="${stockData.stockLevel}" />
		</div>
	</c:otherwise>
</c:choose>
