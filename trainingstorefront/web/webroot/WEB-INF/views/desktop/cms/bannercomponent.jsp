...
<div class="disp-img simple-banner">
	<c:choose>
		...
		<c:otherwise>
			<a href="${encodedUrl}"> <span class="title"> <strong>${headline}</strong>
			</span> <span class="thumb"> <img title="${headline}"
					alt="${media.altText}" src="${media.url}">
			</span> <span class="details"> ${content} </span> <span class="action">
					<theme:image code="img.iconArrowCategoryTile"
						alt="${media.altText}" />
			</span>
			</a>
		</c:otherwise>
