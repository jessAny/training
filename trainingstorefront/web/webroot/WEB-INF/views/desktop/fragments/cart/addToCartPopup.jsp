...
<c:forEach items="${cartData.entries}" var="cartEntry"
	varStatus="status">
	{
		...
		"categories": [
		<c:forEach items="${cartEntry.product.categories}" var="category"
		varStatus="categoryStatus">
			"<c:out value='${category.name}' />"<c:if
			test="${not categoryStatus.last}">,</c:if>
	</c:forEach>
		]
	}<c:if test="${not status.last}">,</c:if>
</c:forEach>
] }, "cartAnalyticsData":{"cartCode" :
"${cartCode}","productPostPrice":"${entry.basePrice.value}","productName":"
...
<spring:escapeBody javaScriptEscape="true">
	...
	<div id="addToCartLayer">
		<div class="cart_popup_error_msg">
			<c:choose>
				<c:when
					test="${multidErrorMsgs ne null and not empty multidErrorMsgs}">
		...
		<div class="legend">
			...
			<c:if
				test="${numberShowing > 0 and fn:length(products) > numberShowing}">
				<p class="legend">
					<spring:theme code="popup.cart.showing"
						arguments="${numberShowing},${fn:length(products)}" />
		...
		<c:choose>
			<c:when test="${modifications ne null}">
				<c:forEach items="${modifications}" var="modification"
					...
					<cart:popupCartItems entry="${entry}" product="${product}"
						quantity="${quantity}" />
				</c:forEach>
			</c:when>
			<c:otherwise>
