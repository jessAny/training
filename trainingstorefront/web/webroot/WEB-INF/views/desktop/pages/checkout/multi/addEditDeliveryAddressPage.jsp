...
<template:page pageTitle="${pageTitle}" hideHeaderLinks="true">
	...
	<div class="span-14 append-1">
		<div id="checkoutContentPanel" class="clearfix">
			...
			<div class="headline">
				<spring:theme code="checkout.multi.addressDetails"
			...
			<div class="description">
				<spring:theme code="checkout.multi.addEditform"
			...
			<c:if test="${not empty deliveryAddresses}">
				<div id="savedAddressListHolder" class="clear">
					<div id="savedAddressList" class="summaryOverlay clearfix">
						<div class="headline">
							<spring:theme code="checkout.multi.deliveryAddress.addressBook"
								text="Address Book" />
						</div>
						<div class="addressList">
							<c:forEach items="${deliveryAddresses}" var="deliveryAddress"
								...
								<div class="addressEntry">
									<form
										action="${request.contextPath}/checkout/multi/delivery-address/select"
										method="GET">
										<input type="hidden" name="selectedAddressCode"
											value="${deliveryAddress.id}" />
										<ul>
											<li>${fn:escapeXml(deliveryAddress.title)}&nbsp;
											...
											<li>${fn:escapeXml(deliveryAddress.town)}&nbsp;
												${fn:escapeXml(deliveryAddress.postalCode)}</li>
											<li>${fn:escapeXml(deliveryAddress.country.name)}<c:if
													test="${not empty deliveryAddress.region.name}">&nbsp; ${fn:escapeXml(deliveryAddress.region.name)}</c:if></li>
										</ul>
										<button type="submit" class="positive left"
											tabindex="${status.count + 21}">
											<spring:theme
												code="checkout.multi.deliveryAddress.useThisAddress"
												text="Use this delivery address" />
										</button>
									</form>
									<c:if
										test="${empty removeAddressEnabled or removeAddressEnabled}">
										<form:form
											action="${request.contextPath}/checkout/multi/delivery-address/remove"
											method="POST">
											<input type="hidden" name="addressCode"
												value="${deliveryAddress.id}" />
											<button type="submit"
												class="negative remove-payment-item left"
												tabindex="${status.count + 22}">
												<spring:theme code="checkout.multi.deliveryAddress.remove"
													text="Remove" />
											</button>
										</form:form>
									</c:if>
