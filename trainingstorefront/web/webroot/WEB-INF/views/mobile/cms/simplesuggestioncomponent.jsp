...
<c:choose>
	<c:when
		...
		<div data-role="collapsible" data-theme="e" data-content-theme="c"
			...
			<div class="referenceGallery swipeGallery" data-enabled='true'
				data-commonresourcepath="${commonResourcePath}">
				<c:forEach end="${component.maximumNumberProducts}"
					items="${suggestions}" var="suggestion" varStatus="varStatus">
					<c:url value="${suggestion.url}/quickView"
						var="productQuickViewUrl" />
					<c:if test="${varStatus.index % 2 == 0}">
						<div class="referencedSlide swipeGallerySlide">
					</c:if>
					<div class="thumb">
						<a href="${productQuickViewUrl}" class="referencedProductImage"
							data-url="${productQuickViewUrl}" data-rel="dialog"
							data-transition="pop"> <product:productPrimaryImage
								product="${suggestion}" format="thumbnail" zoomable="false" />
