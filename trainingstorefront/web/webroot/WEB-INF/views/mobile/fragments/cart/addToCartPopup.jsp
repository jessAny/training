...
{ "cartData": { "total": "${cartData.totalPrice.value}", "products": [
	...
	varStatus="status">
				{
					...
					"categories": [ <c:forEach items="${cartEntry.product.categories}"
		var="category" varStatus="categoryStatus">
										"${category.name}"<c:if test="${not categoryStatus.last}">,</c:if>
	</c:forEach> ]
				}<c:if test="${not status.last}">,</c:if>
</c:forEach>
] }, "cartAnalyticsData":{"cartCode" :
"${cartCode}","productPostPrice":"${entry.basePrice.value}","productName":"${product.name}"}
, "cartPopupHtml": "
<spring:escapeBody javaScriptEscape="true">
	...
	<ycommerce:testId code="cart_popup">
		<div class="dialog_container">
			...
			<div class="dialog_container-content">
				...
				<span class="prod_productimage"> <product:productPrimaryImage
						product="${product}" format="cartIcon" zoomable="false" />
				</span>
				<ycommerce:testId code="cart_popup_product_summary">
					<div class="ui-grid-a addToCartSummary" data-theme="b">
						...
						<ycommerce:testId code="cart_popup_product_quantity">
							...
							<div class="ui-block-b prod_quantity">${quantity}</div>
						</ycommerce:testId>
					</div>
				</ycommerce:testId>
				<div class="clear"></div>
	...
	<ycommerce:testId code="cart_popup_view_bag">
		<a href="${cartUrl}" data-role="button" data-theme="b"> <spring:theme
				code="basket.view.basket" />
		</a>
	</ycommerce:testId>
	</div>
	</div>
