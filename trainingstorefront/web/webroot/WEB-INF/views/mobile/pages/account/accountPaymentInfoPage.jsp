...
<c:if test="${empty paymentInfoData}">
	<p class="emptyMessage">
		<spring:theme code="text.account.paymentDetails.noPaymentInformation"
			text="You have no payment information" />
	</p>
</c:if>
<c:if test="${not empty paymentInfoData}">
	<p>
		<spring:theme code="text.account.paymentDetails.managePaymentDetails"
			text="Manage your saved payment details." />
	</p>
			...
			<div class="ui-grid-a" data-theme="b" data-role="content">
				<div class="ui-block-a" style="width: 100%">
					<ul class="mFormList">
						<li>
							<h3>
								<spring:theme code="text.account.paymentDetails.paymentCard"
									text="Payment Card" />
							</h3>
						</li>
						<c:if test="${paymentInfo.defaultPaymentInfo}">
							<li><spring:theme
									code="text.account.paymentDetails.paymentCard.default"
									text="My Default Payment Card" /></li>
						</c:if>
						<div class="ui-grid-a paymentCardInfoBlock" data-theme="b">
							...
							<c:if test="${not empty paymentInfo.startMonth}">
								...
								<div class="ui-block-b">${paymentInfo.startMonth}/
									${paymentInfo.startYear}</div>
							</c:if>
							...
							<div class="ui-block-b">${paymentInfo.expiryMonth}/
								${paymentInfo.expiryYear}</div>
			...
			<div class="ui-grid-a" data-theme="b" data-role="content">
				<div class="ui-block-a" style="width: 100%">
					<ul class="mFormList">
						...
						<li><b> <c:out
									value="${paymentInfo.billingAddress.title} ${paymentInfo.billingAddress.firstName} ${paymentInfo.billingAddress.lastName}" />
						</b></li>
					...
					<fieldset class="ui-grid-a doubleButton">
						<div class="ui-block-a">
							<c:url value="/my-account/remove-payment-method"
								var="removePaymentCardFormAction" />
							<form:form id="removePaymentCardForm${paymentInfo.id}"
								action="${removePaymentCardFormAction}" method="post"
								class="removePaymentCardForm">
								<input type="hidden" name="paymentInfoId"
									value="${paymentInfo.id}" />
								<a href="#" data-role="button" data-theme="d" data-icon="delete"
									class="removePaymentCardButton" pid="${paymentInfo.id}"
									data-message='<spring:theme code="text.paymentcard.remove.confirm"/>'
									data-headerText='<spring:theme code="text.headertext"/>'> <spring:theme
										code="text.remove" text="Remove" />
								</a>
							</form:form>
						</div>
						<div class="ui-block-b">
							<c:if test="${not paymentInfo.defaultPaymentInfo}">
								<c:url value="/my-account/set-default-payment-details"
									var="setDefaultPaymentDetailsFormAction" />
								<form:form id="setDefaultPaymentDetails${paymentInfo.id}"
									action="${setDefaultPaymentDetailsFormAction}" method="post">
									<input type="hidden" name="paymentInfoId"
										value="${paymentInfo.id}" />
									<c:choose>
										<c:when test="${not paymentInfo.defaultPaymentInfo}">
											<a href="" data-role="button" data-theme="c"
												pid="${paymentInfo.id}" class="setDefaultPayment"> <spring:theme
													code="text.setDefault" text="Set as default" />
											</a>
										</c:when>
										<c:otherwise>
											<a data-role="text" data-theme="c"> <spring:theme
													code="text.default" text="Default" />
											</a>
										</c:otherwise>
									</c:choose>
								</form:form>
