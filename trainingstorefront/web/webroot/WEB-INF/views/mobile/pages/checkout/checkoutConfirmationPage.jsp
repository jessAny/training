...
<template:page pageTitle="${pageTitle}">
	<jsp:body>
		...
		<div class="item_container_holder" data-content-theme="d"
			data-theme="e">
			<div data-theme="b">
				<h3>
					<spring:theme code="checkout.orderConfirmation.success"
						text="Your order was successful!" />
				</h3>
				<h4>
					<spring:theme code="checkout.orderConfirmation.thankYouForOrder"
						text="Thank you for your order" arguments="" />
				</h4>
				<ul class="mFormList">
					<li><spring:theme
							code="checkout.orderConfirmation.orderNumberShort"
							text="Order # {0}" arguments="${orderData.code}" /></li>
					<c:if test="${not empty orderData.statusDisplay}">
						<li><spring:theme
								code="checkout.orderConfirmation.orderStatus"
								text="Order Status: {0}" arguments="${orderData.statusDisplay}" /></li>
					</c:if>
					<li><spring:theme code="text.account.order.orderPlaced"
							text="Placed on {0}" arguments="${orderData.created}" /></li>
				</ul>
			</div>
			<div class="span-20 last" data-theme="b">
				<div data-theme="d">
					<h3 class="summaryHeadline">
						<spring:theme code="checkout.orderConfirmation.summary"
							text="A summary of your order is below:" />
					</h3>
				</div>
				...
				<div class="orderOverviewItems">
					<h6 class="descriptionHeadline">
						<spring:theme code="text.headline.orderitems"
							text="All information about your order items" />
					</h6>
					...
					<div data-theme="b">
						<c:if test="${fn:length(orderData.deliveryOrderGroups) gt 0}">
							...
							<c:forEach items="${orderData.deliveryOrderGroups}"
								var="orderGroup">
					...
					<c:if
						test="${fn:length(orderData.pickupOrderGroups) gt 0 && ycommerce:checkIfPickupEnabledForStore() eq true}">
					<div data-theme="b">
						<h3>
							<spring:theme code="basket.page.title.yourPickUpItems"
									text="Your Pickup Items" />
						</h3>
						<c:forEach items="${orderData.pickupOrderGroups}" var="orderGroup">
							<c:set var="pos"
									value="${orderGroup.entries[0].deliveryPointOfService}" />
							<h4>
									<spring:theme code="basket.page.title.pickupFrom"
										text="Pick Up from\:" />
								</h4>
							<ul>
								<li>${pos.name}</li>
				...
				<li>
					<div class="ui-grid-a doubleButton">
						<sec:authorize ifNotGranted="ROLE_ANONYMOUS">
							<c:set var="buttonStyling"
								value="class='ui-block-a' style='width: 38%;'" />
						</sec:authorize>
						<div ${buttonStyling}>
								<c:url value="/" var="homeUrl" />
								<a href="${homeUrl}" data-role="button" data-theme="d"
						...
						<div class="ui-block-b" style="width: 58%;">
								...
								<a href="${accountOrdersUrl}" data-role="button" data-theme="d"
			...
			<cms:pageSlot position="BottomContent" var="feature" element="div"
				id="bottom-disp-img" class="home-disp-img">
				<cms:component component="${feature}" element="div"
					class="span-24 cms_disp-img_slot" />
			</cms:pageSlot>
		</div>
