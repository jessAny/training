		...
		<c:if test="${hopDebugMode}">
			<div class="item_container">
				<div id="debugWelcome">
					<h3>
						<spring:theme
							code="checkout.multi.hostedOrderPostPage.header.debug" />
					</h3>
				</div>
			</div>
		</c:if>
		<div class="item_container">
			<form:form id="hostedOrderPagePostForm"
				name="hostedOrderPagePostForm"
						...
						<c:forEach items="${hostedOrderPageData.parameters}" var="entry"
							...
							<c:choose>
								<c:when test="${hopDebugMode}">
									<dt>
										<label for="${entry.key}" class="required"> <c:out
												value="${entry.key}" />
										</label>
									</dt>
									<dd>
										<input type="text" id="${entry.key}" name="${entry.key}"
											value="${entry.value}" tabindex="${status.count + 1}" />
									</dd>
								</c:when>
								<c:otherwise>
				...
				<c:if test="${hopDebugMode}">
					<div class="rightcol">
						...
						<input id="button.submit"
							class="submitButtonText show_processing_message" type="submit"
							title="${submitButtonLabel}" value="${submitButtonLabel}" />
					</div>
				</c:if>
			</form:form>
		</div>
