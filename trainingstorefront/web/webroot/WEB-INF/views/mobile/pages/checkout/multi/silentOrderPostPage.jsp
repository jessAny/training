...
<%@ taglib prefix="multi-checkout"
	tagdir="/WEB-INF/tags/mobile/checkout/multi"%>
				...
				<div class="item_container">
								...
								<div class="saved-payment-list-entry">
									<form
										...
										<input type="hidden" name="selectedPaymentMethodId"
											value="${paymentInfo.id}" /> <span
											class="saved-payment-list-item">${fn:escapeXml(paymentInfo.cardType)}</span>
										<span class="saved-payment-list-item">${fn:escapeXml(paymentInfo.cardNumber)}</span>
										<span class="saved-payment-list-item"><spring:theme
												code="checkout.multi.paymentMethod.paymentDetails.expires"
												arguments="${fn:escapeXml(paymentInfo.expiryMonth)},${fn:escapeXml(paymentInfo.expiryYear)}" /></span>
										<span class="saved-payment-list-item">${fn:escapeXml(paymentInfo.billingAddress.firstName)}&nbsp;
											${fn:escapeXml(paymentInfo.billingAddress.lastName)}</span> <span
											class="saved-payment-list-item">${fn:escapeXml(paymentInfo.billingAddress.line1)}</span>
										<span class="saved-payment-list-item">${fn:escapeXml(paymentInfo.billingAddress.postalCode)}&nbsp;
											${fn:escapeXml(paymentInfo.billingAddress.town)}</span>
									...
									<form:form
										action="${request.contextPath}/checkout/multi/payment-method/remove"
					...
					<div class="payment_details_left_col">
						<form:form id="silentOrderPostForm" name="silentOrderPostForm"
							...
							class="create_update_payment_form" action="${paymentFormUrl}"
							method="POST">
							...
							<div class="payment_details_left_col-card">
								<h2>
									<spring:theme
										...
										text="Enter New Payment Details" />
								</h2>
								<div class="infotext">
									<p>
										<spring:theme
								...
								<div class="form_field-elements">
									...
									<formElement:formInputBox idKey="card_nameOnCard"
										...
										inputCSS="text" tabindex="2" mandatory="false" />
									...
									<formElement:formInputBox idKey="card_cvNumber"
										labelKey="payment.cvn" path="card_cvNumber" inputCSS="text"
										mandatory="true" tabindex="3" />
									<template:errorSpanField path="card_startMonth">
										<div id="startDate">
											<dt>
												<label for="card_startMonth"><spring:theme
														code="payment.startDate" /></label>
											</dt>
											<dd>
												<form:select id="card_startMonth" path="card_startMonth"
													cssClass="card_date" tabindex="4">
													<option value=""
														label="<spring:theme code='payment.month'/>" />
													<form:options items="${months}" itemValue="code"
														itemLabel="name" />
												</form:select>

												<form:select id="card_startYear" path="card_startYear"
													cssClass="card_date" tabindex="5">
													<option value=""
														label="<spring:theme code='payment.year'/>" />
										...
										<div class="form_field-label form_field-label-headline">
											<label for="card_expirationMonth"><spring:theme
													code="payment.expiryDate" /></label>
										</div>
										<div class="form_field-input">
											<template:errorSpanField path="card_expirationMonth">
												<formElement:formSelectBox idKey="card_expirationMonth"
													labelKey="payment.month" path="card_expirationMonth"
													mandatory="true" skipBlank="false"
													skipBlankMessageKey="payment.expiryMonth.invalid"
													items="${months}" tabindex="6" itemValue="code" />
											</template:errorSpanField>
											<template:errorSpanField path="card_expirationYear">
												<formElement:formSelectBox idKey="card_expirationYear"
													labelKey="payment.year" path="card_expirationYear"
													mandatory="true" skipBlank="false"
													skipBlankMessageKey="payment.expiryYear.invalid"
													items="${expiryYears}" tabindex="7" itemValue="code" />
											</template:errorSpanField>
									</template:errorSpanField>
								</div>
								...
								<div class="form_field-input">
									<sec:authorize ifNotGranted="ROLE_ANONYMOUS">
										<formElement:formCheckbox
											idKey="checkout.multi.sop.savePaymentInfo"
											labelKey="checkout.multi.sop.savePaymentInfo"
											path="savePaymentInfo" labelCSS="add-address-left-label"
					...
					<div class="payment_details_left_col-billing">
						<div class="title_holder">
							...
							<h2>
								<spring:theme code="text.billingAddress" text="Billing Address" />
							</h2>
						</div>
						<c:if test="${cartData.deliveryItemsQuantity > 0}">
							<formElement:formCheckbox idKey="useDeliveryAddress"
								labelKey="checkout.multi.sop.useMyDeliveryAddress"
								path="useDeliveryAddress" labelCSS="add-address-left-label"
						...
						<address:billAddressFormSelector supportedCountries="${countries}"
							regions="${regions}" tabindex="11" />
					</div>


					<div class="save_payment_details">
						<span class="clear_fix">
							<button data-theme="c" class="form submit_silentOrderPostForm"
								tabindex="19">
								<spring:theme code="mobile.checkout.multi.button.submit"
									text="Submit" />
							</button>
						</span>
					</div>
